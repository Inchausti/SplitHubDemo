...
window.SH_TABLES = {
  ...
  debitos: {
    id: 't-debitos',
    cols: [
      { label: 'RF', tip: 'Registro Fiscal de saída — a unidade de débito. Cada documento emitido gera um de IBS e um de CBS.' },
      { label: 'Tipo Fiscal', tip: 'Qual tributo este registro carrega: <strong>IBS</strong> ou <strong>CBS</strong>.' },
      { label: 'Tipo de DFe', tip: 'Modelo do documento fiscal eletrônico emitido — NF-e, NFS-e, CT-e e demais.' },
      { label: 'Nota Fiscal', tip: 'Número do documento emitido. Clique abre o detalhe.' },
      { label: 'Cliente', tip: 'Destinatário do documento — quem recebe a mercadoria ou o serviço.' },
      { label: 'Data NF', tip: 'Data de emissão, que marca o fato gerador do débito.' },
      { label: 'Valor Total', tip: 'Valor bruto do documento emitido.',    cls: 'r' },
      { label: 'Valor Líquido', tip: 'Valor do documento descontados os tributos.', cls: 'r' },
      { label: 'CBS', tip: 'CBS devida nesta operação de saída.',           cls: 'r' },
      { label: 'IBS', tip: 'IBS devido nesta operação de saída.',           cls: 'r' },
      { label: 'Débito', tip: 'Valor de IBS+CBS a recolher por este registro.',        cls: 'r' },
  ...
  dashDfCp: {
    id: 'dash-df-cp',
    cols: [
      { label: 'DF', tip: 'Documento fiscal de entrada com crédito a apropriar.' },
      { label: 'Fornecedor', tip: 'Emitente do documento.' },
  ...
  dashDfLp: {
    id: 'dash-df-lp',
    cols: [
      { label: 'DF', tip: 'Documento fiscal de entrada com crédito a apropriar, em horizonte mais longo.' },
      { label: 'Fornecedor', tip: 'Emitente do documento.' },
      { label: 'Valor IBS+CBS', tip: 'Crédito em jogo neste documento, somando os dois tributos.', cls: 'r' },
      { label: 'Data NF', tip: 'Data de emissão — início da contagem do prazo de apropriação.' },
...
/* ── Tooltip de cabeçalho de coluna — padrão de projeto ──────────────
   Toda coluna declarada com `tip` no SH_TABLES ganha o mesmo marcador.
   O texto vive junto da definição da coluna, não espalhado pelo HTML de
   cada view, e uma coluna nova só é publicada se souber se explicar.

   O lado do balão segue a posição: colunas da esquerda abrem para a
   direita, e vice-versa, para nenhum balão sair da tabela. */
window.shThTip = function(label, tip, lado) {
  var pos = lado === 'esq' ? 'left:0' : 'right:0';
  return '<span style="display:inline-flex;align-items:center;gap:5px">' + label +
    '<span class="ctr-tip-wrap" style="position:relative;display:inline-flex">' +
    '<span style="width:14px;height:14px;border-radius:50%;background:rgba(var(--teal-alt-rgb),.15);' +
    'border:1px solid rgba(var(--teal-alt-rgb),.35);display:inline-flex;align-items:center;' +
    'justify-content:center;font-size:9px;font-weight:700;color:var(--teal);cursor:default;' +
    'line-height:1;flex-shrink:0">?</span>' +
    '<span class="ctr-tip" style="display:none;position:absolute;' + pos + ';top:22px;width:300px;' +
    'background:var(--sidebar);border:1px solid var(--border);border-radius:8px;padding:12px 14px;' +
    'z-index:9900;box-shadow:0 8px 24px rgba(0,0,0,.28);pointer-events:none;font-weight:400;' +
    'text-transform:none;letter-spacing:0;font-size:11px;color:var(--txt2);line-height:1.6;' +
    'white-space:normal;text-align:left">' + tip + '</span>' +
    '</span></span>';
};
...
window.shRenderThead = function(key) {
  ...
  cfg.cols.forEach(function(col, _i) {
    ...
    else if (col.tip) { th.innerHTML = window.shThTip(col.label, col.tip, _i < _meio ? 'esq' : 'dir'); }
...
window.dashRenderDFsApropriar = function() {
  ...
  var _dashBusca    = ((document.getElementById('dash-df-busca')      ||{}).value||'').toLowerCase();
  var _dashStatusRF = (document.getElementById('dash-df-status-rf')   ||{}).value||'';
  var _dashTipoF    = (document.getElementById('dash-df-tipo-fiscal') ||{}).value||'';
  ...
  (window.nfListaFiltradaGlobal || []).forEach(function(nf) {
    ...
    (nf.registrosFiscais || []).forEach(function(rf) {
      ...
      if (_dashStatusRF && sr !== _dashStatusRF) return;
      if (_dashTipoF && (rf.tipoFiscal || '') !== _dashTipoF) return;
      var v    = rf.valor || 0;
      ...
      var _dashNfLabel = (nf.tipoDF || 'NF-e') + ' ' + (rf.nfVinculada || nf.numero || '');
      var _dashForn    = rf.entidade || nf.entidade || '—';
      if (_dashBusca && !(_dashNfLabel.toLowerCase().includes(_dashBusca) || _dashForn.toLowerCase().includes(_dashBusca))) return;
      var row = {
        nf:   _dashNfLabel,
        forn: _dashForn,
        valor: v,
        ...
        dataExtincaoCredito: rf.dataExtincaoCredito || null
      };
  ...
  function buildRow(r) {
    ...
    return '<tr>'
      ...
      + '<td class="nowrap">' + (window.shCelulaExtincaoCredito ? window.shCelulaExtincaoCredito(r) : '&mdash;') + '</td>'
      + '<td class="nowrap">' + (window.shCelulaPrevExtincao ? window.shCelulaPrevExtincao(r) : '&mdash;') + '</td>'
      + '</tr>';
  }

  var empty = function(msg) { return '<tr><td colspan="9" style="text-align:center;color:var(--txt3);padding:20px">'+msg+'</td></tr>'; };
...
window.abrirDetalhesNFporNumero = function(nfNumero) {
  ...
  (r.registrosFiscais || []).forEach(function(rf) {
    ...
    rfsHtml += '<div style="background:rgba(var(--blue-rgb),.04);border:1px solid rgba(var(--blue-rgb),.15);border-radius:8px;padding:12px;margin-bottom:10px">'
      ...
      + '<div style="color:var(--txt3)" title="Fim do prazo de 5 anos — LC 214/2025, art. 54">Limite art. 54: ' + (window.shCelulaPrevExtincao ? window.shCelulaPrevExtincao(rf) : '&mdash;') + '</div>'
      + '</div>'
      + '</div>';
  });
...
window._rfGerarHistorico = function(rf, nf) {
  ...
  if (_sc_hist === 'glosado') {
    var _glEv = (window.shGlosa && window.shGlosa.historicoRF) ? window.shGlosa.historicoRF(rf) : [];
    if (_glEv.length) _glEv.forEach(function(e) { ev.push(e); });
    else ev.push(mkEv(MK(A(d0,10),'10:45'), 'INCONSISTÊNCIA','Créditos',                  'Fisco',
...
window.abrirDetalheRF = function(rfId) {
  ...
  var html = '<div id="rf-detalhe-overlay" class="moverlay" onclick="if(event.target===this)document.getElementById(\'rf-detalhe-overlay\').remove()">'
    ...
    + '<div class="mbox-divider"></div>'
    + '<div class="mbox-section-label">Ciclo do Crédito</div>'
    + DR('Apropriação', window.shCelulaApropriacao ? window.shCelulaApropriacao(rf) : '—')
    + DR('Extinção do Crédito', window.shCelulaExtincaoCredito ? window.shCelulaExtincaoCredito(rf) : '—')
    + DR('Limite art. 54', window.shCelulaPrevExtincao ? window.shCelulaPrevExtincao(rf) : '—')
    + '<div class="mbox-info-box"><span class="mbox-info-box-label">As três datas</span>'
    + '<span style="color:var(--txt2)"><strong>Apropriação</strong> é a data do recolhimento que extinguiu o '
    + 'débito e fez o crédito nascer (<strong>art. 47</strong>) — no Split Payment, a liquidação financeira da '
    + 'nota; no RAD, o pagamento da guia; no Fornecedor, o vencimento do tributo. '
    + '<strong>Extinção do Crédito</strong> é quando ele foi consumido. '
    + '<strong>Limite art. 54</strong> é o fim do prazo de 5 anos, contado do primeiro dia do período de '
    + 'apuração seguinte ao da apropriação — não da emissão do documento.</span></div>'
    + '<div class="mbox-divider"></div>'
    + '<div class="mbox-section-label">Inconsistências Vinculadas</div>'
...
window._filtrosCreditos = {
  ...
  valMin: '', valMax: '', vlMin: '', vlMax: '', cbsMin: '', cbsMax: '', ibsMin: '', ibsMax: ''
};
...
window.injetarFiltrosCreditos = function() {
  ...
  window.shBuildFilterPanel({
    ...
    fields: [
      ...
      { label: 'Valor Líquido — máx (R$)',id: 'fc-vl-max',  type: 'number', placeholder: '∞', min: 0 },
      { label: 'CBS — mín (R$)',          id: 'fc-cbs-min', type: 'number', placeholder: '0', min: 0 },
      { label: 'CBS — máx (R$)',          id: 'fc-cbs-max', type: 'number', placeholder: '∞', min: 0 },
      { label: 'IBS — mín (R$)',          id: 'fc-ibs-min', type: 'number', placeholder: '0', min: 0 },
      { label: 'IBS — máx (R$)',          id: 'fc-ibs-max', type: 'number', placeholder: '∞', min: 0 }
    ]
  });
};
...
window.creditosFiltrarGrid = function() {
  ...
  f.vlMax   = (document.getElementById('fc-vl-max')  || {}).value || '';
  f.cbsMin  = (document.getElementById('fc-cbs-min') || {}).value || '';
  f.cbsMax  = (document.getElementById('fc-cbs-max') || {}).value || '';
  f.ibsMin  = (document.getElementById('fc-ibs-min') || {}).value || '';
  f.ibsMax  = (document.getElementById('fc-ibs-max') || {}).value || '';
...
window.renderizarTabelaCreditos = function() {
  ...
  // Uma linha por RF (Registro Fiscal)
  var listaRFs = [];
  (window.nfListaFiltradaGlobal || []).forEach(function(nf) {
    if (nf.tipo !== 'entrada') return;
    (nf.registrosFiscais || []).forEach(function(rf) {
      ...
      // Lê o registro do recolhimento. A derivação que existia aqui
      // (dia da NF + 5, travado em 28) ignorava método e contrato,
      // divergia da apropriação e podia cair antes da própria emissão.
      var _pagVal = _temPag ? rf.dataPagamento : '—';
      listaRFs.push({
        rfId: rf.id,
        rf: rf.id,
        ...
        dataNF: rf.data || nf.data || '',
        data: (rf.data || nf.data || '').split('-').reverse().join('/'),
  ...
  if (f.mesAno || (f.mesAnoArr && f.mesAnoArr.length) || f.busca || f.tipoFiscal || f.status || f.contrato || f.metodo ||
      ...
      f.valMin || f.valMax || f.vlMin || f.vlMax || f.cbsMin || f.cbsMax || f.ibsMin || f.ibsMax ||
    ...
    listaRFs = listaRFs.filter(function(r) {
      ...
      if (f.vlMax  !== '' && !isNaN(parseFloat(f.vlMax))  && r.valorLiq   > parseFloat(f.vlMax))  return false;
      if (f.cbsMin !== '' && !isNaN(parseFloat(f.cbsMin)) && r.cbs        < parseFloat(f.cbsMin)) return false;
      if (f.cbsMax !== '' && !isNaN(parseFloat(f.cbsMax)) && r.cbs        > parseFloat(f.cbsMax)) return false;
      if (f.ibsMin !== '' && !isNaN(parseFloat(f.ibsMin)) && r.ibs        < parseFloat(f.ibsMin)) return false;
      if (f.ibsMax !== '' && !isNaN(parseFloat(f.ibsMax)) && r.ibs        > parseFloat(f.ibsMax)) return false;
      return true;
    });
  }
  ...
  if (!listaRFs.length) {
    h = '<tr><td colspan="20" style="text-align:center;color:var(--txt3);padding:24px">Nenhum registro fiscal encontrado para este filtro.</td></tr>';
  } else {
...
window.atualizarKPIsCreditos = function(listaRFs) {
  ...
  (listaRFs || []).forEach(function(r) {
    ...
    else if (sc === 'glosado')         { glosado  += v; glosadoN++; }
  ...
  var _glCausas = 0;
  try { _glCausas = (window.shGlosa && window.shGlosa.fichas || []).length; } catch (e) {}
  set('cred-glosado-sub',   pct(glosado, totalCred) + ' do total · ' + glosadoN + ' RF' + (glosadoN === 1 ? '' : 's')
    + (_glCausas ? ' · ' + _glCausas + ' glosa' + (_glCausas === 1 ? '' : 's') + ' com causa' : ' · crédito negado pelo Fisco'));
  // Status do Registro
...
window.atualizarInteligencia = function() {
  ...
  (function() {
    ...
    var xMin = 0, xMax = 105;
    var allPrazos = pontos.map(function(p) { return p.prazo; });
    var yMin = Math.max(5,  Math.min.apply(null, allPrazos) - 10);
    ...
    var dots = pontos.map(function(p) {
      return { p: p, cx: xp(p.score), cy: ypp(p.prazo), r: rDot(p.vol) };
    });

    // Anti-sobreposição: empurra bolhas que se sobrepõem (10 iterações)
    var minGap = 2;
    for (var iter = 0; iter < 10; iter++) {
      for (var i = 0; i < dots.length; i++) {
        for (var j = i + 1; j < dots.length; j++) {
          var dx = dots[j].cx - dots[i].cx;
          var dy = dots[j].cy - dots[i].cy;
          var dist = Math.sqrt(dx * dx + dy * dy) || 0.1;
          var minDist = dots[i].r + dots[j].r + minGap;
          if (dist < minDist) {
    ...
    // Clamp dentro do plotArea
    dots.forEach(function(d) {
      d.cx = Math.max(padL + d.r, Math.min(W - padR - d.r, d.cx));
      d.cy = Math.max(padT + d.r, Math.min(H - padB - d.r, d.cy));
    });

    var s = '<svg viewBox="0 0 ' + W + ' ' + H + '" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:' + H + 'px;display:block">';
    s += '<defs>'
       + '<filter id="glow-r" x="-50%" y="-50%" width="200%" height="200%"><feGaussianBlur stdDeviation="3" result="blur"/><feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge></filter>'
    ...
    [20, 40, 60, 80, 100].forEach(function(v) {
      var gx = xp(v);
      if (gx < padL || gx > W - padR) return;
      s += '<line x1="' + gx + '" y1="' + padT + '" x2="' + gx + '" y2="' + (H - padB) + '" stroke="rgba(128,128,128,0.1)" stroke-width="1"/>';
      s += '<text x="' + gx + '" y="' + (H - padB + 14) + '" text-anchor="middle" fill="var(--txt3)" font-size="9" font-family="Inter,system-ui,sans-serif">' + v + '</text>';
    });

    // Grid horizontal (prazo) — ticks dinâmicos
    var yTicks = [];
    for (var yt = Math.ceil(yMin / 15) * 15; yt <= yMax; yt += 15) yTicks.push(yt);
    yTicks.forEach(function(v) {
      var gy = ypp(v);
      s += '<line x1="' + padL + '" y1="' + gy + '" x2="' + (W - padR) + '" y2="' + gy + '" stroke="rgba(128,128,128,0.1)" stroke-width="1"/>';
      s += '<text x="' + (padL - 5) + '" y="' + (gy + 3.5) + '" text-anchor="end" fill="var(--txt3)" font-size="9" font-family="Inter,system-ui,sans-serif">' + v + 'd</text>';
    });
    ...
    dots.slice().sort(function(a, b) { return b.r - a.r; }).forEach(function(d) {
      var p = d.p, cx = d.cx, cy = d.cy, r = d.r;
      ...
      var tipData = [p.nome + ' · prazo ' + p.prazo + 'd', cor, 'Score', p.score + '/100', cor, 'Vol. total', fmtV2(p.vol).replace('R$ ','')].join('|');
      var enc = tipData.replace(/'/g, '&apos;');
      s += '<circle cx="' + cx + '" cy="' + cy + '" r="' + (r + 4) + '" fill="' + cor + '" opacity="0.05"/>';
      ...
      // Label: até 2 palavras, máx 14 chars
      var words = p.nome.split(' ');
      var lbl = words.length > 1 ? (words[0] + ' ' + words[1]).substring(0, 14) : words[0].substring(0, 14);
      var lblY = cy + r + 10;
      if (lblY > H - padB - 2) lblY = cy - r - 3;
      s += '<text x="' + cx + '" y="' + lblY + '" text-anchor="middle" fill="' + cor + '" font-size="8" font-weight="600" font-family="Inter,system-ui,sans-serif" opacity="0.9">' + lbl + '</text>';
    });
...
window.renderizarRFsInconsistencias = function() {
  ...
  var _incStatuses = ['aberta','aberta','em_analise','aguardando_emitente','aberta','resolvida','resolvida','aberta','em_analise','aberta'];
...
window.incRfFiltrar = function() {
  ...
  var valTotMin = (document.getElementById('inc-rf-val-total-min') ||{}).value||'';
  var valTotMax = (document.getElementById('inc-rf-val-total-max') ||{}).value||'';
  var valLiqMin = (document.getElementById('inc-rf-val-liq-min')   ||{}).value||'';
  var valLiqMax = (document.getElementById('inc-rf-val-liq-max')   ||{}).value||'';
  ...
  var lista = (window._inconsistenciasGlobal || []).filter(function(inc) {
    ...
    if (valTotMin !== '' && !isNaN(parseFloat(valTotMin)) && (inc.valorTotal||0) < parseFloat(valTotMin)) return false;
    if (valTotMax !== '' && !isNaN(parseFloat(valTotMax)) && (inc.valorTotal||0) > parseFloat(valTotMax)) return false;
    if (valLiqMin !== '' && !isNaN(parseFloat(valLiqMin)) && (inc.valorLiq||0)   < parseFloat(valLiqMin)) return false;
    if (valLiqMax !== '' && !isNaN(parseFloat(valLiqMax)) && (inc.valorLiq||0)   > parseFloat(valLiqMax)) return false;
...
window._incRfRenderPagina = function() {
  ...
  pag.forEach(function(inc) {
    ...
    h += '<tr>'
      ...
      + '<td class="nowrap">' + (window.shCelulaExtincaoCredito ? window.shCelulaExtincaoCredito(inc) : '&mdash;') + '</td>'
...
window.injetarFiltrosDebitos = function() {
  ...
  window.shBuildFilterPanel({
    ...
    fields: [
      ...
      { label: 'Valor Total — mín (R$)',   id: 'fd-val-min', type: 'number', placeholder: '0', min: 0 },
      { label: 'Valor Total — máx (R$)',   id: 'fd-val-max', type: 'number', placeholder: '∞', min: 0 },
      { label: 'Valor Líquido — mín (R$)', id: 'fd-vl-min',  type: 'number', placeholder: '0', min: 0 },
      { label: 'Valor Líquido — máx (R$)', id: 'fd-vl-max',  type: 'number', placeholder: '∞', min: 0 },
      { label: 'CBS — mín (R$)',           id: 'fd-cbs-min', type: 'number', placeholder: '0', min: 0 },
      { label: 'CBS — máx (R$)',           id: 'fd-cbs-max', type: 'number', placeholder: '∞', min: 0 },
      { label: 'IBS — mín (R$)',           id: 'fd-ibs-min', type: 'number', placeholder: '0', min: 0 },
      { label: 'IBS — máx (R$)',           id: 'fd-ibs-max', type: 'number', placeholder: '∞', min: 0 }
    ]
  });
};
...
window.debitosFiltrarGrid = function() {
  ...
  f.valMin  = (document.getElementById('fd-val-min') || {}).value || '';
  f.valMax  = (document.getElementById('fd-val-max') || {}).value || '';
  f.vlMin   = (document.getElementById('fd-vl-min')  || {}).value || '';
  f.vlMax   = (document.getElementById('fd-vl-max')  || {}).value || '';
  f.cbsMin  = (document.getElementById('fd-cbs-min') || {}).value || '';
  f.cbsMax  = (document.getElementById('fd-cbs-max') || {}).value || '';
  f.ibsMin  = (document.getElementById('fd-ibs-min') || {}).value || '';
  f.ibsMax  = (document.getElementById('fd-ibs-max') || {}).value || '';
...
window.debitosLimparFiltrosGrid = function() {
  ['fd-busca','fd-tipo','fd-status','fd-status-reg','fd-contrato','fd-metodo','fd-extincao','fd-data-de','fd-data-ate','fd-deb-min','fd-deb-max','fd-tipo-dfe','fd-val-min','fd-val-max','fd-vl-min','fd-vl-max','fd-cbs-min','fd-cbs-max','fd-ibs-min','fd-ibs-max'].forEach(function(id) {
    var el = document.getElementById(id); if (el) el.value = '';
  });
...
window.renderizarTabelaDebitos = function() {
  ...
  if (!listaRFs.length) {
    h = '<tr><td colspan="20" style="text-align:center;color:var(--txt3);padding:24px">Nenhum RF de débito encontrado para este filtro.</td></tr>';
  }
...
window.debitosDFsRender = function() {
  ...
  var ibsMin   = (document.getElementById('deb-dfs-ibs-min')    ||{}).value||'';
  var ibsMax   = (document.getElementById('deb-dfs-ibs-max')    ||{}).value||'';
  var cbsMin   = (document.getElementById('deb-dfs-cbs-min')    ||{}).value||'';
  var cbsMax   = (document.getElementById('deb-dfs-cbs-max')    ||{}).value||'';
  var filtCont = (document.getElementById('deb-dfs-contrato')   ||{}).value||'';
  // Preencher select de contratos dinamicamente (só na primeira vez)
  (function(){var sel=document.getElementById('deb-dfs-contrato');if(!sel||sel.options.length>2)return;var ids=[];(_contratosData||[]).forEach(function(c){if(ids.indexOf(c.id)<0)ids.push(c.id);});ids.sort().forEach(function(id){var o=document.createElement('option');o.value=id;o.textContent=id;sel.appendChild(o);});})();
  ...
  if(!nfs.length){
    el.innerHTML='<tr><td colspan="16" style="text-align:center;color:var(--txt3);padding:24px">Nenhum DF encontrado para este filtro.</td></tr>';
    return;
  }
  ...
  nfs.forEach(function(nf){
    ...
    h+='<td class="nowrap">'+(window.shCelulaApropriacao?window.shCelulaApropriacao(nf):'&mdash;')+'</td>';
...
window._filtrosPagamentos = { mesAno: '', mesAnoArr: [], tipo: '', busca: '', valorMin: '', valorMax: '', dataRFDe: '', dataRFAte: '', pagamento: '', tipoDFe: '', contrato: '', statusCred: '', statusReg: '' };
...
window.pagamentosFiltrarGrid = function() {
  ...
  f.contrato  = (document.getElementById('fp-contrato')    || {}).value || '';
  f.statusCred= (document.getElementById('fp-status-cred') || {}).value || '';
  f.statusReg = (document.getElementById('fp-status-reg')  || {}).value || '';
  window.renderizarTabelaPagamentos();
};

window.pagamentosLimparFiltros = function() {
  ['fp-busca','fp-tipo','fp-pagamento','fp-data-de','fp-data-ate','fp-valor-min','fp-valor-max','fp-tipo-dfe','fp-contrato','fp-status-cred','fp-status-reg','fp-origem','fp-retorno'].forEach(function(id) {
    var el = document.getElementById(id);
    if (el) el.value = '';
  });
  window._filtrosPagamentos = { mesAno: '', mesAnoArr: (window._filtrosPagamentos.mesAnoArr||[]).slice(), tipo: '', busca: '', valorMin: '', valorMax: '', dataRFDe: '', dataRFAte: '', pagamento: '', tipoDFe: '', contrato: '', statusCred: '', statusReg: '' };
  window.renderizarTabelaPagamentos();
};
...
window.renderizarTabelaPagamentos = function() {
  ...
  (window.nfListaFiltradaGlobal || []).forEach(function(nf) {
    ...
    (nf.registrosFiscais || []).forEach(function(rf) {
      ...
      var _pContratoId = rf.contratoId || nf.contratoId || null;
      if (f.contrato === '__sem__' && _pContratoId) return;
      if (f.contrato && f.contrato !== '__sem__' && _pContratoId !== f.contrato) return;
      if (f.statusCred && _scP !== f.statusCred) return;
      var _pSreg = rf.statusRegistro || null;
      if (f.statusReg && _pSreg !== f.statusReg) return;
      ...
      var dataFmt = dp.length === 3 ? dp[2] + '/' + dp[1] + '/' + dp[0] : '—';
      // Mesma regra da listagem de Créditos: lê o registro, não deriva.
  ...
  // Filtros de origem e return_code. A entrega ao ERP saiu da listagem:
  // ela e um evento do RF e vive no historico, com data e tentativa.
  ...
  if (!rows.length) {
    h = '<tr><td colspan="22" style="text-align:center;color:var(--txt3);padding:24px">Nenhum pagamento RAD encontrado para este filtro.</td></tr>';
  }
